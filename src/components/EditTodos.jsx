      ...
      <div className="panel panel-default">
        ...
        <ul className = "list-group">
          {props.todosArray.map(todo => {
            return <li className={todo.completed ? "strike-through alert alert-success" : (todo.priority === 1 
              ? "alert alert-info" : todo.priority === 2 ? "alert alert-warning" : "alert alert-danger")} key ={todo.id}>
                <input className="form-check-input"type="checkbox" onChange= {() => props.onDone(todo)}/>{todo.text}
                  <a onClick={() => props.onDelete(todo)}
                    className="delete-todo glyphicon glyphicon-trash pull-right" href="#"></a><a onClick={() => props.onEdit(todo)}
                      className="edit-todo glyphicon glyphicon-edit pull-right" href="#">
                  </a>
            </li>
          })}
        </ul>
